...
function disableScan() {
  console.log('disableScan() called ');
...
function scanReceived(params) {
  console.log('scanReceived() params ' + params);
...
function ScanSubmit() {
  EB.Barcode.stop();
  EB.Barcode.start();
  console.log('ScanSubmit() executed.');
}
...
function ajaxRequest(valstr, selectionstr) {
  ...
  xhttp.onreadystatechange = function() {
    if (this.readyState == 4) {
      if (this.status == 200) {
        if (this.responseText.indexOf(AJAX_HEADER) != -1) {
          var sections = this.responseText.split(AJAX_HEADER);
          
          $('form').SequenceId.value = sections[0].trim();
          $('style').innerHTML = sections[1].trim();
          $('h1').innerHTML = sections[2].trim();
          $('.content').innerHTML = sections[3].trim();
          $('nav').innerHTML = sections[4].trim();
          afterPageLoaded();
  ...
  var params = '&XMLHttpRequest=true';
